...
class Settings:
  ...
  def __init__(self, parent=None):
    ...
    self.dbfile           = "/var/lib/pyblaster/pyblaster.sqlite"
                            # database file for device and playlist storage
...
"""
    ...
    try:
      ...
      self.parent.log.write(log.EMERGENCY,
        "Settings.new_config(): Failed to write config file %s" % self.configfile)
      raise
    f.write(msg)
  ...
  def read_config(self):
    ...
    try:
      ...
      self.parent.log.write(log.EMERGENCY,
                            "Failed to open config file %s" % self.configfile)
      raise
